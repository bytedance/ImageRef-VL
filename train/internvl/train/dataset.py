...
try:
    from .tos_client import Client as TosClient
    from bytedtos.errors import TosException
    from requests.exceptions import Timeout, ConnectionError
    import yaml
    import time
except ImportError as E:
    print('bytedtos is not installed. If you read data locally instead of from tos, ignore it.')

import sys


def safe_tos_get_object(client, key, url_prefix):
    obj = key[len(url_prefix):]
    while True:
        try:
            bytes = client.get_object(obj).data
            return bytes
...
def read_frames_gif(
        ...
        client=None, min_num_frames=4, url_prefix=None
):
    if 's3://' in video_path:
        video_bytes = client.get(video_path)
        gif = imageio.get_reader(io.BytesIO(video_bytes))
    elif "tos" in video_path:
        video_bytes = safe_tos_get_object(client, video_path, url_prefix)
        gif = imageio.get_reader(io.BytesIO(video_bytes))
    else:
...
def read_frames_decord(
        video_path, num_frames, sample='rand', fix_start=None,
        client=None, clip=None, min_num_frames=4, url_prefix=None
):
    if 's3://' in video_path:
        video_bytes = client.get(video_path)
...
def read_frames_folder(
    ...
    if 's3://' in video_path:
        ...
        # TODO: replace with tos list obj API
        image_list = sort_frames(safe_tos_get_object(client, video_path, url_prefix))
        frames = []
        for image in image_list:
            fp = os.path.join(video_path, image)
            frame = Image.open(io.BytesIO(client.get(fp)))
...
class TOSLoader(object):
    def __init__(self, conf_path):
        print(f'[TOSLoader] config_path: {conf_path}')
        print('--> before Client(conf_path)')
        ...
        self.client = TosClient(
            config["bucket_name"],
            config["ak"], service=config["tos_psm"],
            cluster=config["tos_cluster"],
            idc=config["tos_idc"],
    ...
    def __call__(self, fn, image_type='image', max_num_frames=-1, min_num_frames=4, sample='rand', clip=None):
        if image_type == 'image':
            img_value_str = safe_tos_get_object(self.client, fn, self.url_prefix)
            img = pil_loader(img_value_str)
            return img
            ...
            if fn.endswith('/'):
                frames = read_frames_folder(fn, num_frames=max_num_frames, min_num_frames=min_num_frames,
                                            client=self.client, sample=sample, url_prefix=self.url_prefix)
            elif fn.endswith('.gif'):
                frames = read_frames_gif(fn, num_frames=max_num_frames, min_num_frames=min_num_frames,
                                         client=self.client, sample=sample, url_prefix=self.url_prefix)
            else:
                frames = read_frames_decord(fn, num_frames=max_num_frames, min_num_frames=min_num_frames,
                                            client=self.client, sample=sample, clip=clip, url_prefix=self.url_prefix)
            return frames
