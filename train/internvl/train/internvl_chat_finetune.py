...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      ...
                                      REF_START_TOKEN)
from internvl.train.dataset import (ConcatDataset, TCSLoader, TOSLoader,
...
class LazySupervisedDataset(Dataset):
    ...
    def load_image(self, image_path):
        ...
        elif self.tos_loader is not None and "tos" in image_path:
            return self.tos_loader(image_path)
        return Image.open(image_path).convert('RGB')

    def get_image_path(self, image_path):
        if image_path.startswith('s3://'):  # for ceph
            image_path = self.root + image_path
        elif "tos" in image_path:
        ...
        while True:
            try:
                ...
                if 'image' in data_item:
                    if type(data_item['image']) == list:
                        if len(data_item['image']) > 0 and data_item['image'][0].startswith('s3://'):
                            images = [self.root + item for item in data_item['image']]
                        elif len(data_item['image']) > 0 and "tos" in data_item['image'][0]:
                        ...
                        elif "tos" in data_item['image']:
                            data_path = data_item['image']
                        else:
...
def main():
    ...
    tos_loader = TOSLoader('tos.yaml') if has_tos_loader else None
    ...
    train_dataset = build_datasets(
        data_args, tokenizer, tcs_loader, tos_loader, model, group_by_length=training_args.group_by_length,
